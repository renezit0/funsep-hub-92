...
async function generateIRReport(supabase: any, beneficiary: any, matricula: number, dataInicio: string, dataFim: string) {
  try {
    const exercicio = new Date(dataInicio).getFullYear()  // Exercício (ano que declara)
    const anoCalendario = exercicio - 1  // Ano-calendário (ano dos rendimentos)
    
    console.log('Gerando relatório IR:', { matricula, exercicio, anoCalendario })
    console.log(`Exercício ${exercicio} - Ano-calendário ${anoCalendario}`)
    ...
    try {
      const { data: irTitularIRPFD, error: irTitularIRPFDError } = await supabase
        ...
        .select('*')
        .eq('matricula', matricula)
        .eq('ano', anoCalendario)  // Usar ano-calendário (exercício - 1)
    ...
    const htmlContent = generateIRReportHTML(
      ...
      dependentes,
      irDependentes,
      exercicio,  // Passar o exercício para o HTML
      anoCalendario  // Passar o ano-calendário para o HTML
    )
    
    return new Response(JSON.stringify({ 
      html: htmlContent,
      filename: `IR_${beneficiary.nome.replace(/[^A-Z0-9]/gi, '_')}_${matricula}_${exercicio}.pdf`
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json',
...
<body>
    ...
    <div class="declaracao-text">
        ...
        exercício de ${exercicio}/Ano-Calendário ${anoCalendario}, pagou ao FUNSEP - CNPJ 20.601.112/0001-91, o valor de R$ 
